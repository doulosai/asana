...
export async function getCheckoutDates(propertyId, fromDate, toDate) {
  const data = await hospitable(
    `/v1/properties/${propertyId}/reservations?start_date=${fromDate}&end_date=${toDate}`
  );

  const reservations = data.data || data || [];

  // Extract checkout dates — try common field names
  const checkouts = reservations
    .map(r => r.check_out || r.checkout_date || r.end_date || r.checkOut)
    .filter(Boolean)
    .map(d => d.split('T')[0])
